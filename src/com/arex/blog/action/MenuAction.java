...
public class MenuAction extends CommonAction<MenuDTO> {
	...
	public String home() {
		...
		List<BlogDTO> blogDTOList = blogService
				.searchAllBlog((HttpServletRequest) request);
	...
	public String photo() {
		...
		// photoService.searchAllPhotoByUserId(((UserDTO)session.getAttribute("loginUser")).getUserId());
		// 启用分页查询
		List<PhotoDTO> photoDTOList = photoService.searchAllPhotoByUserId(
				(HttpServletRequest) request,
				((UserDTO) session.getAttribute("loginUser")).getUserId());
		// 设置到request中去
	...
	public String blog() {
		...
		if (isLogin) {
			// 已经登录，显示个人博客列表
			// blogDTOList =
			// blogService.searchAllBlogByUserId(((UserDTO)session.getAttribute("loginUser")).getUserId());
			blogDTOList = blogService.searchAllBlogByUserId(
					(HttpServletRequest) request,
					((UserDTO) session.getAttribute("loginUser")).getUserId());
		} else {
			// 没有登录，显示所有博客列表
		...
		List<CategoryDTO> categoryDTOList = categoryService
				.searchAllCategoryByUserId(((UserDTO) session
						.getAttribute("loginUser")).getUserId());
		for (CategoryDTO categoryDTO : categoryDTOList) {
			// int counts =
			// blogService.searchBlogCountsByCategoryId(categoryDTO.getCategoryId());
			int counts = blogService.searchBlogCountsByCategoryIdAndUserId(
	...
	public String message() {
		...
		// messageService.searchAllMessageByReceiverId(loginUser.getUserId());
		// //获取新消息
		// //启用分页查询新消息
		// // List<MessageDTO> messageDTOListWithNew =
		// messageService.searchAllMessageByReceiverIdAndMessageStatus(loginUser.getUserId(),
		// 1);
		// List<MessageDTO> messageDTOListWithNew =
		// messageService.searchAllMessageByReceiverIdAndMessageStatus((HttpServletRequest)request,
		// loginUser.getUserId(), 1);
		// //获取已读消息
		// //启用分页查询已读消息
		// if (messageDTOListWithNew.size() <
		// Integer.parseInt((String)request.getAttribute("pageSize"))) {
		// request.setAttribute("pageSize",
		// Integer.parseInt((String)request.getAttribute("pageSize"))-messageDTOListWithNew.size());
		// List<MessageDTO> messageDTOListWithRead =
		// messageService.searchAllMessageByReceiverIdAndMessageStatus((HttpServletRequest)request,
		// loginUser.getUserId(), 2);
		// request.setAttribute("messageDTOListWithRead",
		// messageDTOListWithRead);
		// }
		// // List<MessageDTO> messageDTOListWithRead =
		// messageService.searchAllMessageByReceiverIdAndMessageStatus(loginUser.getUserId(),
		// 2);
		...
		// 启用分页查询
		List<MessageDTO> messageDTOList = messageService
				.searchAllMessageByReceiverId((HttpServletRequest) request,
						loginUser.getUserId());
		request.setAttribute("messageDTOList", messageDTOList);
		request.setAttribute(
				"messages",
				messageService.searchAllMessageByReceiverIdAndMessageStatus(
						loginUser.getUserId(), 1).size());
	...
	public String sendMessageDisplayPage() {
		...
		// 启用分页查询
		List<MessageDTO> messageDTOList = messageService
				.searchAllMessageBySenderId((HttpServletRequest) request,
						loginUser.getUserId());
		request.setAttribute("messageDTOList", messageDTOList);
		request.setAttribute(
				"messages",
				messageService.searchAllMessageBySenderIdIdAndMessageStatus(
						loginUser.getUserId(), 1).size()
						+ messageService
								.searchAllMessageBySenderIdIdAndMessageStatus(
										loginUser.getUserId(), 2).size());
	...
	public String messageDetail() {
		...
		if (menuDTO == null || menuDTO.getMessageId() == null
				|| "".equals(menuDTO.getMessageId())) {
		...
		MessageDTO messageDTO = messageService.searchMessageByMessageId(menuDTO
				.getMessageId());
		// 设置到request中
	...
	public String personalPage() {
		...
		if (super.getModel().getKind() != null
				&& "edit".equals(super.getModel().getKind())) {
			// 启用编辑
	...
	public String blogDetailPage() {
		...
		List<CommentDTO> commentDTOList = commentService
				.searchAllCommentByBlogId(blogId);
		// 生成具有层次的评论
		...
		for (int i = 0; commentDTOList != null && i < commentDTOList.size(); ++i) {
			...
			while (commentDTO != null && commentDTO.getParentId() != null
					&& !"".equals(commentDTO.getParentId())) {
				commentDTO = commentService.searchCommentByCommentId(commentDTO
						.getParentId());
				// commentDTO = this.getCommentDTOByCommentId(commentDTOList,
	...
	@Deprecated
	private CommentDTO getCommentDTOByCommentId(List<CommentDTO> commentList,
			String parentId) {
	...
	public String deleted() {
		List<BlogDTO> blogDTOList = blogService.searchAllDeletedBlogByUserId(((UserDTO) session.getAttribute("loginUser")).getUserId());

		// 设置当前登陆用户的分类列别
		List<CategoryDTO> categoryDTOList = categoryService.searchAllDeletedCategoryByUserId(((UserDTO) session.getAttribute("loginUser")).getUserId());
		for (CategoryDTO categoryDTO : categoryDTOList) {
			int counts = blogService.searchDeletedBlogCountsByCategoryIdAndUserId(categoryDTO.getCategoryId(),((UserDTO) session.getAttribute("loginUser")).getUserId());
			categoryDTO.setCounts(counts);
		}
		// 将categoryDTOList设置到application中
		application.setAttribute("deletedCategoryDTOList", categoryDTOList);
